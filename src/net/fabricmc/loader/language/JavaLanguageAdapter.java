...
public class JavaLanguageAdapter implements LanguageAdapter {
	private static boolean canApplyInterface(String itfString) throws IOException {
		...
		InputStream stream = Launch.knotLoader.getClassLoader().getResourceAsStream(LoaderUtil.getClassFileName(itfString));
	...
	public static Class<?> getClass(String className, Options options) throws ClassNotFoundException, IOException {
		InputStream stream = Launch.knotLoader.getClassLoader().getResourceAsStream(LoaderUtil.getClassFileName(className));
		...
		stream.close();
		return Launch.knotLoader.getClassLoader().loadClass(className);
	}
